# ============================================================================
# SCAPeSCLC Diagnostic Atlas Generator for Biological Pathways (BPs)
# ============================================================================
#
# Description:
#   Generates multi-page diagnostic dashboards for BP-level Cox proportional
#   hazards models using standardized bayesian posteriors of pathway activity
#   from SCAPeSCLC.
...
#
# Output:
#   SCAPeSCLC Diagnostic Atlas of BPs (Adjusted) - <endpoint>.pdf
#
# Endpoints:
#   OS
#   DSS
...
# Read Source Data
df <- read.csv(file = "D13_patient_BP_posteriors.csv",
...
colnames(df) <- colnames(df) %>%
  str_remove("\\s*\\(Posterior\\)$")

# ==========
# BP Columns
# ==========
BP_COLUMNS <- seq(from = 22, to = 337, by = 3)
bp_cols <- colnames(df)[BP_COLUMNS]
...
extract_model_summary <- function(model,
  ...
  hr <- sprintf("%.3f", result$estimate)
  ci <- paste0(
    sprintf("%.3f", result$conf.low),
    "–",
    sprintf("%.3f", result$conf.high)
  )
  cox_p <- sprintf("%.3f", result$p.value)
...
make_summary_grob <- function(model_summary, bp) {
...
#     zph  : cox.zph object
#----------------------------------------------------
make_schoenfeld_plot <- function(model, bp, endpoint) {
  ...
  term_names <- gsub("`", "", rownames(zph$table), fixed = TRUE)
  idx <- which(term_names == bp)
...
#     plot : ggplot object
# ---------------------------------------------------
make_dfbeta_plot <- function(model, bp, endpoint) {
  
  dfb <- residuals(model, type = "dfbeta")
  colnames(dfb) <- names(coef(model))
  
  df_dfb <- data.frame(
    Patient = seq_len(nrow(df)),
    DFBETA = dfb[, match(bp, gsub("`", "", colnames(dfb), fixed = TRUE))]
  )
...
# ------------------------------------------------------------------
make_diagnostic_dashboard <- function(bp,
                                      endpoint,
                                      time_var,
                                      event_var) {
  ...
  formula <- as.formula(
    sprintf(
      "Surv(%s, %s) ~ `%s` + treatment + bone_metastasis",
      time_var,
      event_var,
      bp
    )
...
=====================================\n",
  ...
  cat(
    "Endpoint: ", endpoint, "\n",
    "Genes: ", length(bp_cols), "\n",
    "Output: SCAPeSCLC Diagnostic Atlas of BPs (Adjusted) - ", endpoint,
    ".pdf\n\n",
    "Please wait...\n\n",
    sep = ""
  )
  ...
  cairo_pdf(
    file = paste0("SCAPeSCLC Diagnostic Atlas of BPs (Adjusted) - ",
                  endpoint,
                  ".pdf"
    ),
    width = 8,
